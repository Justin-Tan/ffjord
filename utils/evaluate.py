...
class Evaluator:
    ...
    def __call__(self, data_loader, identifier="disVAE", is_metrics=False):
        ...
        metrics_filename = "metrics_{}_{:%Y_%m_%d_%H:%M}.log".format(identifier, datetime.datetime.now())

        if is_metrics:
            self.logger.info('Computing metrics...')
            metrics, entropies = self.compute_metrics(data_loader)
    ...
    def _compute_q_zCx(self, dataloader):
        ...
        try:
            ...
            latent_dim = self.model.module.latent_dim
        n_suff_stat = 2

        q_zCx_dist = distributions.Normal()
        q_zCx = torch.zeros(len_dataset, latent_dim, n_suff_stat, device=self.device)

        n = 0
        pbar = tqdm(total=len_dataset)
        with torch.no_grad():
            for x, label in dataloader:
                batch_size = x.size(0)
                idcs = slice(n, n + batch_size)
                try:
                    q_zCx[idcs, :, 0], q_zCx[idcs, :, 1] = self.model.encoder(x.to(self.device, torch.float))['continuous']
                except AttributeError:
                    q_zCx[idcs, :, 0], q_zCx[idcs, :, 1] = self.model.module.encoder(x.to(self.device, torch.float))['continuous']
                n += batch_size
                pbar.update(batch_size)
        pbar.close()

        params_zCX = q_zCx.unbind(-1)
        samples_zCx = q_zCx_dist.sample(*params_zCX)
